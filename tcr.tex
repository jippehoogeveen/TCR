...
% General settings:
\setminted{fontsize=\small, baselinestretch=0.9, breaklines, tabsize=2, pygmentize = C:/Users/jippe/AppData/Roaming/Python/Python313/Scripts/pygmentize.exe,  latexminted = C:/Users/jippe/AppData/Local/Programs/Python/Python313/Scripts/latexminted.exe}
% Referencing files:
\newcommand{\mintedstyle}[2]{\inputminted{#1}{code/#2}}
\newcommand{\code}[1]{ Hash: \input{code/#1.hash}
\mintedstyle{cpp}{#1}}
...
\newcommand{\QQ}{ll twee = 1; -- Utrecht University }

% Header/Footer
%\geometry{includeheadfoot}
%\fancyhf{}
\pagestyle{fancy}
\lhead{Utrecht University, \QQ}
\chead{}
\rhead{\thepage/25}
\cfoot{}
...
\title{TCR}
\subtitle{\textbf{\QQ} \\ \textit{Thomas van der Plas \& Jippe Hoogeveen \& Felix Hamoen}}
...
\max(0,\left\lfloor \frac{C-Ax}{B} + 1\right\rfloor)$. Be very careful about overflows.
...
\begin{myitemize}
	...
	$1031$, $32771$, $1048583$, $33554467$, $998244353$, $9982451653$, $1073741827$, $34359738421$, $1099511627791$, $35184372088891$, $1125899906842679$, $36028797018963971$, $10^{18} + 7$.
...
\item \textbf{Primitive Roots:}
    ...
    $998244352 = 2^{23} \cdot 7 \cdot 17 + 1: 2$, $2.013.265.921 = 2^{27} \cdot 3 \cdot 5 + 1 : 31$,
    $10^9 + 7 : 5$,\ $10^9 + 9 : 13$,\ $10^9 + 21 : 2$,\ $10^9 + 33 : 5$,\ $10^9 + 87 : 3$,\ $36028797018963971 : 2$, $10^{18} + 7 : 5$,\ $(10^9 + 7)^2 : 5$
...
All intervals are right closed $[\ell,r]$.
...
\subsection{Cartesian tree/Treap}

Binary tree derived from a sequence of points. The root is the point with smallest $y$-coordinate and contains the points with smaller $x$ in its left subtree (and larger $x$ in right subtree). \textbf{Can be used as treap (= BST on $x$)}, by adding random $y$ to each point. Additional information can be updated in augment. Can be made persistent by changing the update instructions in split, merge and erase to creating new nodes (\textbf{use keyword new}). 
\code{datastructures/cartesian_tree.cpp}

Construction based on arrays $\mathcal{O}(n)$.
\code{datastructures/cartesian_tree_construction.cpp}
...
A 2-dimensional range tree supporting range queries in $O(\log(n))$ time.
\code{datastructures/range_tree.cpp}
...
\label{sect:lct}
Tree where you can add pseudolines in $O(\log(n))$ and query the maximum line for values $a_1 < a_2 < \cdots < a_n$ in $O(\log(n))$. 2 pseudolines can intersect at most once.
...
\subsubsection{Floyd-Warshall $\mathcal{O}(V^{3})$ all pairs}
...
\subsubsection{Blossom matching $\mathcal{O}(E V^2)$}
\label{sec:bm}
...
Vertices/edges that when removed split their connected component in two. Leaves are true in iscut and not in cp. \textbf{Looping over all blocks and then over all edges can be quadratic!}
\code{graphs/cpbs_decomposition.cpp}
...
\textbf{NOTE: Not sure if it works correctly with disconnected graphs or produces the correct min cut. DOES NOT WORK FOR DIRECTED GRAPHS.}
...
Finds an Euler Path (or circuit) in a \emph{undirected} graph:
...
this on each strongly connected component. $\mathcal{O}(EV)$ runtime.
...
\subsection{Manacher algorithm $\mathcal{O}(n)$}
Returns longest palindrome centered at letter $i$ (index $2 \cdot i$) or between letters $i$ and $i + 1$ (index $2 \cdot i + 1$).
\code{strings/manacher.cpp}
...
% Tested on https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=1620
...
DP examples
\code{strings/suffix_automaton_dp.cpp}
...
NTT $\mathcal{O}(n\log(n))$. Requires $2 ^{e_2(mod - 1)} \geq n$. Can be calculated exact by taking (multiple) large primes as modulus (and combining them with CRT).
\code{math/ntt.cpp}
...
\begin{myitemize}
	...
	\item TLE?
	\begin{myitemize}
		\item Replace endl with newline
		\item Replace vector with const size array
		\item \#pragma GCC optimize("O3,unroll-loops") \#pragma GCC target("avx2,bmi,bmi2,popcnt")
		\item for loop with const upper bound $N \leq 1000 \rightarrow$ \#pragma GCC unroll($N$)
		\item Replace conditional assignment with $a = c \cdot t + !c \cdot f$ / ternary when $t$/$f$ computationally expensive 
		\item Reduce modulo usage / replace with with bit operations 
		\item Still not enough? Try to reduce asymptotic runtime
	\end{myitemize}
\end{myitemize}
